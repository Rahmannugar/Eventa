...
API Gateway, Identity, Event, and Commerce use NestJS/TypeScript. Ticket, Discovery, Analytics, and Notification use Go. Analytics and Notification are event-driven Go services.
...
Identity publishes `attendee.deleted.v1`, and Event Service publishes `event.published.v1`, through their service-owned outboxes. Ticket Service and Commerce consume the shared Event lifecycle Kafka topic for `event.cancelled.v1`: Ticket revokes issued tickets, Commerce claims eligible paid orders for refund, and each owns a durable inbox keyed by the message ID. Both acknowledge other lifecycle facts without acting on them. No deployable consumes `attendee.deleted.v1` or reacts to `event.published.v1`; each remaining consumer must be introduced by the product story that owns its reaction, durable inbox/idempotency boundary, recovery policy, and operating owner. RabbitMQ consumers are separate workers for assigned jobs and are not Kafka business-fact consumers.
