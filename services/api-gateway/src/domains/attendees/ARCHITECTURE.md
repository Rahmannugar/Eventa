...
- Credentialed CORS names one configured web-client origin shared by attendee and admin routes. Login, logout, deletion, forgot-password, and reset-password reject any other present browser origin. Cookie attributes remain host-only, HttpOnly, `SameSite=Lax`, and `Secure` on HTTPS.
