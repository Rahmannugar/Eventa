...
Signed-in attendees retrieve their issued tickets and QR credentials with `GET /tickets`. The Gateway derives attendee identity from the server-backed session and forwards bounded pagination to Ticket Service. Missing or invalid sessions return `401`; unavailable Ticket Service returns `503`.
