...
The attendee ticket-options route authenticates through Identity and supplies the resolved attendee ID to a dedicated Event query. Gateway strictly validates event and currency ownership, type uniqueness, timestamps, and the lifecycle-specific availability shape before grouping types by currency for HTTP. It does not calculate availability or receive exact inventory counters. Event Service owns queue priority, offer protection, reservation visibility, and public-surplus calculation. The later reservation command rechecks those rules under the ticket-type lock.
...
Cancellation has a tighter separate abuse budget. Gateway forwards the authenticated admin ID, event ID, expected version, and request correlation under the Event deadline. Event Service alone decides whether the event is published and performs the atomic cancel transition, capacity release, audit, and outbox insert. Gateway maps a non-published event to `422 EVENT_CANCELLATION_NOT_ALLOWED` and a stale version to `409`.
...
Ticket-currency definition and ticket-type creation, update, and retirement use dedicated Event gRPC operations. Gateway validates transport shape, derives the admin ID from the session, applies separate read and mutation budgets, propagates the request ID, and bounds each call with the Event deadline. Event Service owns currency grouping, price, capacity, reserved and sold quantities, schedule, parent ownership, uniqueness, lifecycle, version, transaction, and audit rules. Gateway verifies event and ticket identity plus the capacity equation before exposing the catalogue to the client.
