...
Ticket-currency definition and ticket-type creation use dedicated Event gRPC operations. Gateway validates transport shape, derives the admin ID from the session, applies separate read and mutation budgets, propagates the request ID, and bounds each call with the Event deadline. Event Service owns currency grouping, price, capacity, schedule, parent ownership, uniqueness, count, draft-state, version, transaction, and audit rules. Gateway verifies that every returned currency and ticket type belongs to the requested event before exposing the catalogue to the client.
