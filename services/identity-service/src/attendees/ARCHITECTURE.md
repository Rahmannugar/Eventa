...
2. The controller creates `RegisterAttendeeCommand` and invokes the injected `RegisterAttendeeHandler` capability.
3. `ObservedRegisterAttendeeCommandHandler` wraps the core handler and records the authoritative outcome.
4. `RegisterAttendeeCommandHandler.handle()` canonicalizes email and username and asks the `PasswordHasher` capability for an Argon2id hash.
5. `AttendeeAccountWriteRepository` receives canonical fields and the hash through `AttendeeAccountWriter`.
