...
import type {
  EmailClient,
  SendEmail,
} from '../../src/notifications/ports/email.client';
...
describe('EmailVerificationEmailSender', () => {
  it('owns verification content and passes the job ID as client idempotency key', async () => {
    const client = new RecordingEmailClient();
    ...
    expect(client.messages[0]?.html).toContain('123456');
    expect(client.messages[0]?.text).toContain('123456');
  });
});
