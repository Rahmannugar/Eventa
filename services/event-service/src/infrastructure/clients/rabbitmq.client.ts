...
export class RabbitMQClient implements OnApplicationShutdown, OnModuleInit {
  ...
  consumerChannel(purpose: string): Promise<Channel> {
    return this.channel(purpose, false);
  }

  confirmChannel(purpose: string): Promise<ConfirmChannel> {
  ...
  private async createChannel(
    ...
    channel.on('close', () => {
      if (this.channels.get(purpose) === channel) this.channels.delete(purpose);
    });
