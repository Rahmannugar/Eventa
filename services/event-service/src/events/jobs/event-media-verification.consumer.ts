...
import {
  ...
  EVENT_MEDIA_VERIFICATION_OPERATION,
  EVENT_MEDIA_VERIFICATION_QUEUE,
} from '../constants/event-media.constants';
import type { EventMediaVerificationService } from '../services/event-media-verification.service';

export class EventMediaVerificationConsumer
  implements OnModuleInit, OnApplicationShutdown
{
  private channel: Channel | undefined;
  private consumerTag: string | undefined;
  private readonly logger = new Logger(EventMediaVerificationConsumer.name);
  private restartTimer: NodeJS.Timeout | undefined;
  private shuttingDown = false;

  constructor(
    private readonly rabbitMQ: RabbitMQClient,
    private readonly verifier: EventMediaVerificationService,
  ) {}
  ...
  private async start(): Promise<void> {
    const channel = await this.rabbitMQ.consumerChannel(
      'event-media-verification-job-consumer',
    );
    ...
    this.logger.log({
      event: 'event_media_verification_consumer_ready',
      prefetch: EVENT_MEDIA_CONSUMER_PREFETCH,
      queue_name: EVENT_MEDIA_VERIFICATION_QUEUE,
    });
  ...
  private async handle(channel: Channel, message: Message): Promise<void> {
    ...
    addJobInFlight(1, { operation: EVENT_MEDIA_VERIFICATION_OPERATION });
    try {
      while (!this.shuttingDown && this.channel === channel) {
        try {
          const outcome = await context.with(parent, () =>
            runWithOperationSpan(
              'event_media_verification_job.process',
              () => this.process(channel, message),
              {
                attributes: {
                  'messaging.destination.name': EVENT_MEDIA_VERIFICATION_QUEUE,
          ...
          recordJobMetrics(
            ...
            { operation: EVENT_MEDIA_VERIFICATION_OPERATION, outcome },
          );
          return;
        } catch (error: unknown) {
          this.logger.error({
            error_type: error instanceof Error ? error.name : 'UnknownError',
            event: 'event_media_verification_consumer_error',
            operation: EVENT_MEDIA_VERIFICATION_OPERATION,
          });
  ...
  private async process(channel: Channel, message: Message): Promise<string> {
    const uploadId = this.validate(message);
    if (uploadId === undefined) {
      this.logger.error({
        event: 'event_media_verification_job_rejected',
        operation: EVENT_MEDIA_VERIFICATION_OPERATION,
      });
      channel.ack(message);
      return 'rejected';
    ...
    const outcome = await this.verifier.verify(uploadId);
    ...
    const fields = {
      event: 'event_media_verification_completed',
      job_id: uploadId,
      message_id: uploadId,
      operation: EVENT_MEDIA_VERIFICATION_OPERATION,
  ...
  private validate(message: Message): string | undefined {
    try {
      if (
        ...
        message.properties.type !== EVENT_MEDIA_VERIFICATION_JOB_TYPE
      )
        return undefined;
      ...
      const uploadId = Reflect.get(value, 'uploadId') as unknown;
      const type = Reflect.get(value, 'type') as unknown;
      return type === EVENT_MEDIA_VERIFICATION_JOB_TYPE &&
  ...
  private scheduleRestart(): void {
    ...
    this.restartTimer = setTimeout(() => {
      ...
      void this.start().catch((error: unknown) => {
        this.logger.error({
          error_type: error instanceof Error ? error.name : 'UnknownError',
          event: 'event_media_verification_consumer_restart_failed',
        });
