...
`ListEventTicketTypes` accepts an event ID and returns its ticket currencies, current event version, and at most 20 ticket types in stable currency and creation order. Missing and retired events return `NOT_FOUND`.
