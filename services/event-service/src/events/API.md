...
`CreateEventTicketType` accepts the acting admin, event ID, expected event version, normalized name, optional description, ISO 4217 currency, non-negative integer price in minor units, allocation, and sales window. The event must be an active draft. Sales must end after they start and no later than the event start. The first ticket type fixes the event currency; later types must use it. Names are unique per event without case sensitivity, and an event may have at most 20 ticket types. Creation increments the event version and appends an audit record in the same transaction. A stale version returns `ABORTED`; invalid or conflicting configuration returns `INVALID_ARGUMENT`; a published event or reached limit returns `FAILED_PRECONDITION`.

`ListEventTicketTypes` accepts an event ID and returns the event currency, current event version, and at most 20 ticket types in stable creation order. Missing and retired events return `NOT_FOUND`.
